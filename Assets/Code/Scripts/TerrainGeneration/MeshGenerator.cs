			...
			for (int x = 0; x < numVertsPerLine; x++) {
				...
				if (!isSkippedVertex) {
					...
					if (isEdgeConnectionVertex) {
                        ...
                        EdgeConnectionVertexData edgeConnectionVertexData = new EdgeConnectionVertexData(vertexIndex, vertexIndicesMap[coordA.x, coordA.y], vertexIndicesMap[coordB.x, coordB.y], distancePercentFromAToB);
...
public class MeshData {
	...
	private void FlatShading() {
		Vector3[] floatShadedVertices = new Vector3[triangles.Length];
		Vector2[] flatShadedUVs = new Vector2[triangles.Length];

		for (int i = 0; i < triangles.Length; i++) {
			floatShadedVertices[i] = vertices[triangles[i]];
			flatShadedUVs[i] = uvs[triangles[i]];

			triangles[i] = i;
		}
		vertices = floatShadedVertices; 
		uvs = flatShadedUVs;
